...
class GyemsDRC:
    ...
    def __init__(self, can_bus=None, device_id=0x141, units='rad'):
        ...
        state_labels = ['temp', 'angle', 'speed', 'torque', 'current', 'zero_angle']
        self.state = dict(zip(state_labels, [0, 0, 0, 0, 0, 0]))
    ...
    def recive_reply(self):  # should be receive_reply
    ...
    def enable(self, clear_errors=False):
        ...
        command = self.protocol[
                      'motor_running'] + 7 * b'\x00'  # message = {0x141:  b'\x81\x00\x00\x00\x00\x00\x00\x00'}
        self.send_command(command)
        self.recive_reply()

        self.set_current(0)
        self.state['zero_angle'] = self.state['angle']
        self.state['angle'] = 0
    ...
    def parse_state(self, reply):
        ...
        # self.time = time.time()
        self.state['angle'] = self.angle_scale * self.multiturn_encoder(self.raw_state_data['encoder']) - self.state[
            'zero_angle']
    ...
    def set_angle(self, angle, speed_limit=None):
        ...
        if self.speed_limit:
            self.command = self.protocol['set_pos_2'] + b'\x00' + self.to_bytes(2, self.speed_limit) + self.to_bytes(4,
                                                                                                                     self.desired_angle)
        else:
